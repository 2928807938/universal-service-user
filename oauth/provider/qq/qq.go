...
type Provider struct {
	config *provider.QQConfig
}

// New 创建 QQ OAuth 提供者
func New(config *provider.QQConfig) *Provider {
	return &Provider{config: config}
}

// GetName 获取平台唯一标识
func (p *Provider) GetName() string { return "qq" }

// GetDisplayName 获取平台显示名称
func (p *Provider) GetDisplayName() string { return "QQ" }

// GetAuthURL 生成授权跳转 URL
func (p *Provider) GetAuthURL(ctx context.Context, state string) (string, error) {
	return fmt.Sprintf("https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id=%s&redirect_uri=%s&state=%s&scope=get_user_info",
...
func (p *Provider) ValidateConfig() error {
	if p.config == nil || p.config.AppID == "" || p.config.AppKey == "" {
		return fmt.Errorf("QQ 配置不完整")
	}
	return nil
}
